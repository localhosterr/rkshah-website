...
@section('page-subtitle','Add, edit, delete packages · Image upload · Destinations · Includes')
...
.img-preview{width:70px;height:50px;object-fit:cover;border-radius:var(--r);border:1px solid var(--border)}
</style>
@endpush
...
<div style="background:#FEF3C7;border:1px solid #FDE68A;border-radius:var(--r2);padding:12px 18px;margin-bottom:20px;display:flex;gap:10px;align-items:center">
  ...
  <div style="font-size:12px;color:#92400E">
    <strong>Image upload works in both Add and Edit.</strong>
    Changed fields turn green. Each package has its own Save button. 🗑️ permanently deletes.
  </div>
</div>
  ...
  <div class="card">
    ...
    <div class="card__body">
      <form action="{{ route('cms.content.packages.update',$pkg->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr auto;gap:10px;align-items:end;margin-bottom:12px">
          ...
          <div class="form-group" style="margin:0">
            ...
            <input class="form-input js-track" type="number" name="price"
                   value="{{ $pkg->price }}" min="0" placeholder="0 = custom">
          ...
          <div class="form-group" style="margin:0">
            ...
            <input class="form-input js-track" type="text" name="badge"
                   value="{{ $pkg->badge }}" placeholder="e.g. Best Seller">
        ...
        <div class="form-row" style="margin-bottom:12px">
          <div class="form-group" style="margin:0">
            <label class="form-label">Destinations <span style="color:var(--t4);font-weight:400">(comma separated)</span></label>
            ...
            <div class="current-val">📍 {{ is_array($pkg->destinations) ? implode(', ',$pkg->destinations) : '—' }}</div>
          </div>
          <div class="form-group" style="margin:0">
            <label class="form-label">Includes <span style="color:var(--t4);font-weight:400">(comma separated)</span></label>
        ...
        <div style="background:var(--bg);border-radius:var(--r2);padding:12px;border:1px solid var(--border)">
          ...
          <div style="display:grid;grid-template-columns:60px 1fr 1fr;gap:10px;align-items:start">
            ...
            <div class="form-group" style="margin:0">
              <label class="form-label">Upload Image <span style="color:var(--t4);font-weight:400">(800×220px · max 4MB)</span></label>
    ...
    <form action="{{ route('cms.content.packages.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="modal__body">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label class="form-label">Emoji</label>
            <input class="form-input" type="text" name="emoji" placeholder="🎒"
                   maxlength="5" value="🎒" style="font-size:20px;text-align:center">
          </div>
        </div>
        ...
        <div class="form-group">
          <label class="form-label">Package Image <span style="color:var(--t4);font-weight:400">(optional · 800×220px · max 4MB · jpg/png/webp)</span></label>
          <input type="file" name="package_image" accept="image/jpg,image/jpeg,image/png,image/webp"
        ...
        <div class="form-group">
          <label class="form-label">What's Included <span style="color:var(--t4);font-weight:400">(comma separated)</span></label>
          <input class="form-input" type="text" name="includes"
                 value="AC Cab, Expert Driver, Fuel &amp; Tolls">
        </div>
